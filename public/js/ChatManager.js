import { sendChat as apiSendChat } from '/scripts/effect.api.js';
...
export class ChatManager {
  ...
  async sendMessage(messageOverride) {
    ...
    const message = (typeof messageOverride === 'string' ? messageOverride : null) || (messageInput?.value?.trim() || '');
    if (!message) return;
  ...
  async sendToEmotionEngine(message) {
    ...
    const data = await apiSendChat({
      ...
      emotionState: this.emotionState || null
    });
