export class TTSManager {
  ...
  async callTTSAPI(cleanText, useStreaming = false) {
    const ttsProvider = this.configManager ? this.configManager.getTTSProvider() : 'elevenlabs';
    ...
    if (!voiceId) {
      console.error('❌ Voice ID not loaded from server (.env). Please ensure ConfigManager.loadConfig() has been called.');
      throw new Error('Voice ID not configured. Please check your .env file and server configuration.');
    }
    ...
    if (ttsProvider === 'elevenlabs') {
      ...
      try {
        console.log(`🎤 Calling ElevenLabs TTS API (streaming) for "${cleanText.substring(0, 50)}..."`);
        ...
        console.log(`🎵 ElevenLabs Response: ${data.streaming ? 'STREAMING' : 'standard'}, ${data.duration}s duration`);
        return data;
      } catch (error) {
    ...
    // Amazon Polly provider (fallback/legacy)
      ...
      try {
        console.log(`🎤 Calling Polly TTS API: ${endpoint} for "${cleanText.substring(0, 50)}..."`);
  ...
  async createAudioObject(data) {
    ...
    if (hasServerVisemes) {
      ...
      if (Math.abs(durationScaleFactor - 1.0) > 0.05) { // Only scale if difference > 5%
        console.log(`⏱️ Duration mismatch detected! Rescaling visemes:`);
        console.log(`   Server estimate: ${serverEstimatedDuration.toFixed(3)}s`);
        ...
        console.log(`⏱️ Duration match: ${actualAudioDuration.toFixed(3)}s (server: ${serverEstimatedDuration.toFixed(3)}s)`);
      }
  ...
  actuallyPlayAudio(audioObject, cleanText, data) {
    ...
    if (actualAudioDuration && data.duration && Math.abs(actualAudioDuration - data.duration) > 0.1) {
      console.log(`⏱️ Using actual audio duration: ${actualAudioDuration.toFixed(3)}s (was ${data.duration.toFixed(3)}s)`);
    ...
    if (!data.streaming) {
      console.warn("⚠️ Streaming is disabled - audio will wait for full buffer");
    }
    ...
    console.log('🎤 TalkingHead Input Debug:', {
      ...
      uniqueVisemes: audioObject.visemes ? [...new Set(audioObject.visemes)].slice(0, 10) : 'N/A',
