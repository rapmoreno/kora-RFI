...
export class SentimentAgent extends BaseAgent {
  ...
  async initialize(): Promise<void> {
    ...
    if (this.personalityContext) promptParts.push(this.personalityContext);
    if (linguisticEnginePrompt) promptParts.push(linguisticEnginePrompt);
    if (falseMemoryPrompt) promptParts.push(falseMemoryPrompt);
    if (agentPrompt) promptParts.push(agentPrompt);
